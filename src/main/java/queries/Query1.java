...
public class Query1 {
    private static String pathParquet1 = "/data/yellow_tripdata_2021-12.parquet";
    ...
    public static void main(String[] args) {
        ...
        spark.read().csv("hdfs://hdfs-namenode:9000"+pathParquet1);

        JavaRDD<String> rdd =
                //spark.read().csv(finalPath)
                spark.read().csv("hdfs://hdfs-namenode:9000")
                .toJavaRDD().map(
                        row -> row.mkString(",")
        );
